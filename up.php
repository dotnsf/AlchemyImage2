        ...
        if( $result = $stmt->fetch( PDO::FETCH_ASSOC ) ){
          ...
          if( $r == 1 ){
            ...
            if( $result = $stmt->fetch( PDO::FETCH_ASSOC ) ){
              ...
              $apiurl = 'http://access.alchemyapi.com/calls/url/URLGetRankedImageFaceTags?apikey=' . $apikey . '&outputMode=json&url=' . urlencode( $url );
              ...
              if( count( $imageFaces ) ){
                for( $i = 0; $i < count( $imageFaces ); $i ++ ){
                  ...
                  $agescore = $imageFace->age->score;
                  $gender = $imageFace->gender->gender;
                  $genderscore = $imageFace->gender->score;
                  
                  //. tags テーブルに情報を格納
                  $sql = "insert into tags(imeta_id,tag,score) values(:imeta_id,:tag,:score)";
                  $stmt = $dbh->prepare( $sql );
                  $stmt->bindParam( ':imeta_id', $imeta_id, PDO::PARAM_INT );
                  $stmt->bindParam( ':tag', $age, PDO::PARAM_STR );
                  $stmt->bindParam( ':score', $agescore, PDO::PARAM_STR );
                  $stmt->execute();
                  $stmt = $dbh->prepare( $sql );
                  $stmt->bindParam( ':imeta_id', $imeta_id, PDO::PARAM_INT );
                  $stmt->bindParam( ':tag', $gender, PDO::PARAM_STR );
                  $stmt->bindParam( ':score', $genderscore, PDO::PARAM_STR );
                  $stmt->execute();
                }
              }
            }
